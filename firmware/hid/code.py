# Alternate HID token firmware for kde-serial-keylock.
#
# Use with firmware/hid/boot.py. The original firmware/code.py serial token is
# intentionally left unchanged.
#
# HID report payload format, excluding the report ID byte:
#   byte 0: ASCII payload length, 0-126
#   bytes 1..n: ASCII command or response text
...
hid = hid_device()
last_host_report = None


def write_line(line):
    payload = line.encode()
    if len(payload) > REPORT_PAYLOAD_SIZE - 1:
        payload = payload[: REPORT_PAYLOAD_SIZE - 1]
    report = bytearray(REPORT_PAYLOAD_SIZE)
    report[0] = len(payload)
    report[1 : 1 + len(payload)] = payload
    hid.send_report(report, REPORT_ID)


def read_line():
    global last_host_report
    try:
        report = hid.get_last_received_report(REPORT_ID)
    except TypeError:
        report = hid.get_last_received_report()
    if report is None or report == last_host_report:
        return None
    last_host_report = report
    if len(report) == 0:
        return None
    length = report[0]
    if length <= 0:
    ...
    if length > len(report) - 1:
        length = len(report) - 1
    return bytes(report[1 : 1 + length]).decode().strip()
...
def handle_line(line):
    ...
    if len(parts) == 2 and parts[0] == "KEYLOCK/1":
        ...
        write_line("ERR unsupported command")
